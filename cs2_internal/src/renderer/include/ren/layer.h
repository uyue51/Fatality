...
namespace evo::ren
{
	...
	public:
		...
		void add_shadow_color_rect(const rect& r, color c, float radius, bool bg, float a);
		void add_shadow_rect(const rect& r, float radius, bool bg = true, float a = .25f);
		void add_rect_filled_rounded_gradient(const rect& r, color c1, color c2, float amount, rounding rnd);
		void add_path_stroke(const std::vector<vec2>& points, color col, float thickness, bool closed);
		void add_path_arc_to(const vec2& center, float radius, float start_angle, float end_angle, color col, int segments);
		...
		void add_progress_circle_outline(vec2 center, float radius, color c, int segments, float progress, float thickness, outline_mode mode);
