...
namespace sdk
{
	...
	static const CInterfaceRegister* get_register_list(const wchar_t* wszModuleName)
	{
		void* hModule = utils::get_module_base_handle(wszModuleName);
		if (hModule == nullptr)
			return nullptr;

		std::uint8_t* pCreateInterface = reinterpret_cast<std::uint8_t*>(utils::get_export_address(hModule, XOR("CreateInterface")));
