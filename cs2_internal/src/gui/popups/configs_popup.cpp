...
namespace gui::popups
{
	...
	void refresh_configs(bits v)
	{
		...
		for (const auto& f : fs::directory_iterator(DEC_INLINE(sdk::game->game_dir) + XOR("fatality")))
		{
	...
	void create_config(bits v, const std::string& name)
	{
		...
		std::ofstream _file(DEC_INLINE(sdk::game->game_dir) + XOR("fatality/") + name + XOR(".cfg"));
		_file.close();
	...
	void delete_config(bits v)
	{
		...
		const auto file = DEC_INLINE(sdk::game->game_dir) + XOR("fatality/") + cfg.cfg_list[(size_t)v % cfg.cfg_list.size()].name + XOR(".cfg");
		fs::remove(file);
		refresh_configs(v);
#endif
	...
	void save_config(bits v)
	{
		...
		const auto file = DEC_INLINE(sdk::game->game_dir) + XOR("fatality/") + c.name + XOR(".cfg");
		cfg.save(file);
	...
	void load_config(const std::string& key)
	{
		...
		for (auto& i : cfg.cfg_list)
		{
			if (key == i.key)
			{
				cfg.load(DEC_INLINE(sdk::game->game_dir) + XOR("fatality/") + i.name + XOR(".cfg"));
				...
				cfg.currently_loaded = key; // Set the loaded config key
	...
	void load_config_finish()
	{
		...
		// Check if the config was loaded (based on `currently_loaded`)
		if (cfg.currently_loaded.empty())
		{
			return;
		}

		// Reset any flags or perform additional state updates if needed
		ctx->should_update_old_value = true;
		menu::men.full_cycle_reset = true;
