...
struct cfg_t
{
	...
	struct aim_t
	{
		setting<bool> enabled, autofire, anti_exploit, fallback_on_key, hsmode, knife_bot, zeus_bot, slide, fake_duck, punch, doubletap, nospread,hideshot, silent, dormant,
	...
	struct weapon_config_t
	{
		...
		arr_setting<evo::gui::bits> target_selection_mode;
		...
		// make_fmt ¸¨Öúº¯ÊýÊµÏÖ
		template<typename T>
		format_step<T> make_fmt(T value, const char* fmt) {
			return { { value, value }, fmt };
		}

		template<typename T>
		format_step<T> make_fmt(std::pair<T, T> range, const char* fmt) {
			return { range, fmt };
		}
	} weapon_config;
