...
void visuals_t::store()
{
	if (!sdk::resource_service->game_entity_system)
		return;
	...
	for (auto i = 1; i <= 64; i++)
	{
		...
		auto remove = [&](const bool force = false)
			{
				player.alpha -= 255.f / 0.3f * sdk::global_vars->frame_time;
				player.alpha = clamp(player.alpha, 0.f, 255.f);
				player.model_alpha -= 255.f / 0.3f * sdk::global_vars->frame_time;
		...
		const auto player_controller = sdk::resource_service->game_entity_system->get_player_controller(i);
		if (!player_controller || !player_controller->find_class(FNV1A("CCSPlayerController")) || player_controller->get_b_is_local_player_controller())
		{
			remove(true);
			continue;
		}
		...
		if (!player_pawn || !player_pawn->find_class(FNV1A("C_CSPlayerPawn")) || !player_pawn->is_alive())
		{
			remove(true);
			continue;
		}

		if (player_pawn->get_fl_last_spawn_time_index().get_value() != player.spawntime)
		{
			player.spawntime = player_pawn->get_fl_last_spawn_time_index().get_value();
			player.alpha = 0.f;
			player.alpha_lerp = 0.f;
			player.model_alpha = 0.f;
		}

		player.model_alpha += 255.f / 0.5f * sdk::global_vars->frame_time;
		player.model_alpha = clamp(player.model_alpha, 0.f, 255.f);
		player.alpha_lerp += 255.f / 0.6f * sdk::global_vars->frame_time;
		...
		if (cfg.visuals.disablers->test(cfg_t::vis_disable_health_bar_interp) || static_cast<float>(player_pawn->get_i_health()) > player.health_interpolated)
			player.health_interpolated = static_cast<float>(player_pawn->get_i_health());

		player.health_interpolated = math::approach(static_cast<float>(player_pawn->get_i_health()), player.health_interpolated, sdk::global_vars->frame_time * 23.f * std::max(1.f, player.health_interpolated - static_cast<float>(player_pawn->get_i_health())));
		...
		//we have valid player and such lets store some stuff
		player.player_pawn = player_pawn;
		player.player_controller = player_controller;
		player.health = player_pawn->get_i_health();
...
void visuals_t::run(const sdk::cview_setup *view_setup)
{
	...
	for (auto& player : players)
	{
		...
		if (!player.player_pawn)
			continue;

		player.offset = {};

		player.pos = player.player_pawn->as<sdk::C_BaseEntity>()->get_computed_abs_origin();

		auto pos_top = player.pos + sdk::vector{ 0, 0, player.player_pawn->get_vec_view_offset().z + 8 };
		...
		auto player_out_of_fov = [&]()
		{
			...
			if (last_framecount != sdk::global_vars->frame_count)
			{
				if (swap)
					alpha += 255.f / 0.73f * sdk::global_vars->frame_time;
				else
					alpha -= 255.f / 0.73f * sdk::global_vars->frame_time;
				alpha = clamp(alpha, 0.f, 255.f);
				last_framecount = sdk::global_vars->frame_count;
			}
		...
		auto player_info = [&]()
		{
			...
			{
				const auto armor = player.player_pawn->get_armor_value();
				if (armor > 0)
					add_bar(player, esp_item_pos::bottom, {cfg.visuals.esp.armor_color.get()}, armor / 100.f, armor <= 93 ? std::optional(armor) : std::nullopt);
			}
			
			if (cfg.visuals.esp.ammo.get() && player.player_pawn->get_p_weapon_services() && player.player_pawn->get_p_weapon_services()->get_h_active_weapon().get())
			{
				const auto weapon = player.player_pawn->GetActiveWeapon();
				const auto wpn_item = weapon ? &weapon->get_attribute_manager().get_item() : nullptr;
				const auto wpn_data = wpn_item ? player.player_pawn->GetActiveWeapon()->weapon_vdata() : nullptr;
				const auto clip = clamp(player.player_pawn->get_p_weapon_services()->get_h_active_weapon().get()->get_i_clip1(), 0, 1000);
				const auto maxclip = wpn_data->get_i_max_clip1(); // ned weapon v 

				if (clip > 0 && maxclip > 0)
					add_bar(player, esp_item_pos::bottom, { cfg.visuals.esp.ammo_color.get() }, static_cast<float>(clip) / maxclip, static_cast<float>(clip) / maxclip <= 0.93f ? std::optional(clip) : std::nullopt);
			}

			if (cfg.visuals.esp.health.get())
			{
				const auto health = player.health;
				const auto max_health = std::max(health, player.player_pawn->get_i_max_health());
				...
				if (health > 0 && max_health > 0)
					add_bar(player, esp_item_pos::left, colors, player.health_interpolated / max_health, static_cast<float>(health) / max_health <= 0.93f ? std::optional<int>(health) : std::nullopt);
			}

			if (cfg.visuals.esp.ping.get())
			{
				const auto ping = player.player_controller->get_i_ping();
				if (ping >= 100)
			...
			{
				add_text(player, esp_item_pos::top, cfg.visuals.esp.name_color.get(), player.player_controller->get_s_sanitized_player_name().get(), true);
			}

			if (cfg.visuals.esp.flags->test(cfg_t::esp_flag_immune) && player.player_pawn->get_f_immune_to_gun_game_damage_time().get_value() - sdk::global_vars->cur_time > 0.f)
			{
				add_text(
					player, esp_item_pos::right, color(
						color(1.f, 0.552f, 0.21f), std::min(player.player_pawn->get_f_immune_to_gun_game_damage_time().get_value() - sdk::global_vars->cur_time, 1.f)),
					XOR("IMMUNE"));
			}

			if (cfg.visuals.esp.flags->test(cfg_t::esp_flag_money) && player.player_controller->get_p_in_game_money_services())
			{
				add_text(player, esp_item_pos::right, color{130, 180, 0}, XOR("$") + std::to_string(player.player_controller->get_p_in_game_money_services()->get_i_account()));
			}
			if (cfg.visuals.esp.flags->test(cfg_t::esp_flag_armor) && (player.player_controller->get_b_pawn_has_helmet() || player.player_pawn->get_armor_value() > 0))
			{
				const auto string = player.player_pawn->get_b_prev_helmet() ? XOR("HK") : XOR("K");
			...
			if (cfg.visuals.esp.flags->test(cfg_t::esp_flag_scoped) && player.player_pawn->get_b_is_scoped())
			{
				add_text(player, esp_item_pos::right, color(0.18f, 0.451f, 0.788f), XOR("SCOPED"));
			}
			
			const auto weapon = player.player_pawn->get_p_weapon_services() ? player.player_pawn->get_p_weapon_services()->get_h_active_weapon().get() : nullptr;
			const auto wpn_item = weapon ? &weapon->get_attribute_manager().get_item() : nullptr;
			const auto wpn_data = wpn_item ? player.player_pawn->GetActiveWeapon()->weapon_vdata() : nullptr;
			if (cfg.visuals.esp.weapon.get() && wpn_item && wpn_item->as<sdk::cs2_econ_item_view>()->get_item_definition())
			{
				add_text(
					player, esp_item_pos::bottom, cfg.visuals.esp.weapon_color.get(), sdk::loc->find_safe(
			...
			auto has_zeus = false;
			auto has_bomb = false;
			if ((cfg.visuals.esp.flags->test(cfg_t::esp_flag_taser) || cfg.visuals.esp.flags->test(cfg_t::esp_flag_bomb)) && player.player_pawn->get_p_weapon_services() &&
				player.player_pawn->get_p_weapon_services()->get_h_my_weapons().size > 0)
			{
				for (auto idx = 0; player.player_pawn->get_p_weapon_services()->get_h_my_weapons().data[idx].valid() && idx < player.player_pawn->get_p_weapon_services()->
					 get_h_my_weapons().size; idx++)
				{
					const auto wpn = player.player_pawn->get_p_weapon_services()->get_h_my_weapons().data[idx].get();
					if (!wpn)
						continue;

					if (wpn->get_attribute_manager().get_item().get_i_item_definition_index() == sdk::weapon_id::weapon_taser)
						has_zeus = true;

					else if (wpn->get_attribute_manager().get_item().get_i_item_definition_index() == sdk::weapon_id::weapon_c4)
						has_bomb = true;
				}
			}
			...
			{
				const auto amount = fmod(sdk::global_vars->real_time, 0.5f) * 2.f;
				const auto opacity = static_cast<int>(ceilf(sdk::global_vars->real_time)) % 1 ? 1.f - amount : amount;
				const auto zeus_active = wpn_item->get_i_item_definition_index() == sdk::weapon_id::weapon_taser;
				add_icon(
			...
			if (cfg.visuals.esp.flags->test(cfg_t::esp_flag_defuser) && player.player_pawn->get_b_prev_defuser())
			{
