...
namespace menu::group
{
	...
	void rage_aimbot_weapon(std::shared_ptr<layout> &e, uint32_t grp, const std::string &id)
	{
		...
		ADD_RUNTIME(
			...
			-1.f, 
			100.f,
			cfg.weapon_config.hitchance[(int)grp],
			ssa<float>{
			ssv<float>{0.f, XOR("%.0f%%")},   
				ssv<float>{-1.f, XOR("Auto")}   
		},
			1.f,
			vec2{ 80.f, 14.f }
		);
		...
		ADD_RUNTIME(
			...
			0.f,
			125.f,
			cfg.weapon_config.mindmg[(int)grp],
			ssa<float>{
			ssv<float>{101.f, XOR("HP + %.0f")},    // 大于等于 101 时显示 "HP + 数值"
				ssv<float>{1.f, XOR("%.0f hp")},       // 大于等于 1 时显示 "数值 hp"
				ssv<float>{0.f, XOR("Lethal")}         // 等于 0 时显示 "Lethal"
		},
			1.f,
			vec2{ 80.f, 14.f }
		);
		...
		{
			const auto c = MAKE_RUNTIME(id + XOR("target_selection"), combo_box,
				cfg.weapon_config.target_selection_mode[(int)grp],  // 绑定新配置项
				vec2{}, vec2{ 80.f, 20.f });

			c->allow_multiple = false;  // 关键修改：禁用多选
			c->add({
				MAKE_RUNTIME(id + XOR("target_selection>damage"), selectable, XOR("Damage"), 0),  // 值0
				MAKE_RUNTIME(id + XOR("target_selection>crosshair"), selectable, XOR("Crosshair"), 1),  // 值1
				MAKE_RUNTIME(id + XOR("target_selection>mindamage"), selectable, XOR("Mindamage"), 2)  // 值2
				});

			ADD_INLINE("Target selection", c);
		}
		//VIRTUALIZER_TIGER_WHITE_END;
	}
	...
	void rage_aimbot_general(std::shared_ptr<layout> &e)
	{
		...
		ADD("Silent aim", "rage>aimbot>general>silent", checkbox, cfg.aim.silent);
		...
		ADD("Anto fire", "rage>aimbot>general>anti-exploit", checkbox, cfg.aim.anti_exploit);
		ADD("Delay shot", "rage>aimbot>general>target dormant", checkbox, cfg.aim.dormant);
		ADD("Autofire", "rage>aimbot>general>autofire", checkbox, cfg.aim.autofire);
		ADD("Duck peek assist", "rage>aimbot>general>hide shot", checkbox, cfg.aim.hideshot);
		...
		ADD("Zeus bot", "rage>aimbot>general>zeus bot", checkbox, cfg.aim.zeus_bot);
		/* {
		...
		ADD("Nospread", "rage>aimbot>general>nosperad", checkbox, cfg.aim.nospread),
			MAKE("rage>aimbot>general>nosperad>settings", settings, [](auto e) { ADD("Hitchance", "rage>aimbot>general>nospread>settings>hitchance", slider<float>, 0.f,
				100.f, cfg.aim.zeus_hc, XOR("%.0f%%")); });

		ADD("Doubletap", "rage>aimbot>general>double tap", checkbox, cfg.aim.doubletap),
			MAKE("rage>aimbot>general>double tap>settings", settings, [](auto e) { ADD("Hitchance", "rage>aimbot>general>zeus bot>settings>hitchance", slider<float>, 0.f,
				100.f, cfg.aim.zeus_hc, XOR("%.0f%%")); });
	}
