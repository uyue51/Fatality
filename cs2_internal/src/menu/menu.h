...
namespace menu
{
	...
	namespace group
	{
		...
		struct fmt_step {
			// 单值构造函数
			fmt_step(T val, const char* fmt)
				: is_range(false), value(val), format(fmt) {
			}

			// 范围构造函数
			fmt_step(T min, T max, const char* fmt)
				: is_range(true), min(min), max(max), format(fmt) {
			}
		...
		// 工厂函数
		template <typename T>
		fmt_step<T> make_fmt(T val, const char* fmt) {
			return fmt_step<T>(val, fmt);
		}

		template <typename T>
		fmt_step<T> make_fmt(T min, T max, const char* fmt) {
			return fmt_step<T>(min, max, fmt);
		}
	}
